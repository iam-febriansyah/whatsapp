...
async function sendByText(data) {
  ...
  try {
    ...
    if (!exists) {
      updateAfterSendMessage(idMessage, device_id, jid_phone, "", number, message, file, 7, type, "The receiver number is not exists.");
      result = {
    ...
    await sendMessage(session, receiver, { text: message })
      .then(async (response) => {
        ...
        console.log(result);
        updateAfterSendMessage(idMessage, device_id, jid_phone, messageId, number, message, file, messageStatus, type, "");
        return result;
      })
      .catch((err) => {
        result = {
        ...
        updateAfterSendMessage(idMessage, device_id, jid_phone, "", number, message, file, 7, type, err.toString());
        console.log(result);
        return result;
      });
  } catch (err) {
...
async function sendByDoc(data) {
  ...
  if (file != "") {
    ...
    if (response.status == 200) {
      ...
      try {
        ...
        await sendMessage(session, receiver, {
          ...
          .catch((err) => {
            ...
            updateAfterSendMessage(idMessage, device_id, jid_phone, "", number, message, file, 7, type, err == null ? "null" : err.toString());
            console.log(result);
            return result;
          });
      } catch (err) {
      ...
      updateAfterSendMessage(idMessage, device_id, jid_phone, "", number, message, file, 7, type, "URL file tidak ditemukan");
      console.log(result);
      return result;
    }
  } else {
...
async function sendDirectMessage(id) {
  ...
  if (data != null) {
    ...
    if (type == "text") {
      ...
      updateAfterSendMessage(idMessage, device_id, "", "", number, message, file, 7, type, "Message: Unknown type");
    }
  } else {
    result = {
      statusCode: 200,
...
function updateAfterSendMessage(idMessage, device_id, jid_phone, message_id, number, message, url, status_message, type, remarks) {
