...
import makeWASocket, { makeWALegacySocket, useSingleFileAuthState, useSingleFileLegacyAuthState, makeInMemoryStore, Browsers, DisconnectReason, delay } from "@adiwajshing/baileys";
...
const createSession = async (sessionId, isLegacy = false, res = null) => {
  ...
  const { state, saveState } = isLegacy ? useSingleFileLegacyAuthState(sessionsDir(sessionFile)) : useSingleFileAuthState(sessionsDir(sessionFile));
  ...
  const waConfig = {
    ...
    browser: ["Eazynotif Multi Device", "MacOS", "3.0"],
  };
...
const init = () => {
  readdir(sessionsDir(), (err, files) => {
    ...
    for (const file of files) {
      if (!file.endsWith(".json") || (!file.startsWith("md_") && !file.startsWith("legacy_")) || file.includes("_store")) {
        continue;
      }
